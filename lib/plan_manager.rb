	...
	def check_good_plan_restrictions(action, subject)
		if action == "create"
			if subject == "weather_grid"
				...
				if count >= 5
					return "Your plan only allows 5 WeatherGrids. Please visit <a href='/subscriptions'>Plans</a> to buy a plan"
				end
	...
	def check_awesome_plan_restrictions(action, subject)
		if action == "create"
			if subject == "weather_grid"
				...
				if count >= 10
					return "Your plan only allows 10 WeatherGrids. Please visit <a href='/subscriptions'>Plans</a> to buy a plan"
				end
